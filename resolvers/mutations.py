...
def merge_potlukk_details(self, input: PotlukkDetailsMergeInput) -> Potlukk | None:
    if potluk := potlukks.get(input.potlukkId):
        return potluk
        
def swap_potlukk_dishes(self, input: DishesSwapInput) -> Potlukk | None:
    if potluk := potlukks.get(input.potlukkId):
        potluk.dishes = [d.as_dish() for d in input.dishes]
        return potluk

def send_invite(self, input: InvitationSendInput) -> Potlukk | None:
    if potluk := potlukks.get(input.potlukkId):
...
def update_invite(self, input: InvitationUpdateInput) -> Potlukk | None:
    if potluk := potlukks.get(input.potlukkId):
        for invite in potluk.invitations:
